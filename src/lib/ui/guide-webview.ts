...
function buildGuideLabels(options: EasyMailGuideOptions): GuideLabels {
  if (options.locale === "zh-CN") {
    return {
      ...
      sections: [
        ...
        { id: "setup", title: "配置说明", items: ["VS Code Settings 是唯一配置来源；文件夹建议通过选择 Outlook 文件夹命令加载，手写仍可用。", "重点和忽略发件人、关键词安全规则与保留期都在 Settings 中配置，可随时补充或修改。", "Prompt 分类决定模型可返回哪些邮件分类；每项包含 ID、显示名、判断说明和优先级提示。保存后在下一次刷新或分析读取，不会重分类历史结果。", "回复模板决定四个固定草稿段落在本地如何排版。模型只提供段落内容；手动 Generate Draft 始终请求草稿。", "模型列表不会自动刷新，点击加载模型后会缓存到本机，直到你再次手动加载。", "如果 Outlook COM 不可用，先用示例数据验证插件和 Copilot 路径。"] },
  ...
  return {
    ...
    sections: [
      ...
      { id: "setup", title: "Configuration", items: ["VS Code Settings are the single source of truth. Use Select Outlook Folders to load folder choices; manual entry remains available.", "Configure important and ignored senders, keyword safety rules, and retention in Settings whenever they become useful.", "Prompt Categories define which mail categories the model may return. Each entry has an ID, display labels, a decision description, and a priority hint. Changes apply on the next refresh or analysis and do not reclassify existing results.", "The reply template controls the local layout of four fixed draft parts. The model supplies the parts; Generate Draft always requests a draft.", "Model discovery is manual and cached locally until you click Load Copilot Models again.", "If Outlook COM is unavailable, use sample data to validate the extension and Copilot path first."] },
