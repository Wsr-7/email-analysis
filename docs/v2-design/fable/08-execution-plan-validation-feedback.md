...
- **验收**：`npm test` 全绿。**needs user validation**：卸载重装同版本 vsix → Guide 再弹；同一安装内重启 VS Code 不重复弹。
  ...
  - Completion Notes（2026-07-13）：
    - 改动文件：`src/extension.ts`、`src/test/extension-cancellation.test.ts`。
    - 实现边界：Guide key 依次使用非空 Marketplace metadata、扩展目录 `birthtimeMs`、version；未改 Guide 内容、其他 globalState 或打包流程。
...
## 11. Milestone F7 — 第五轮验证反馈批（2026-07-14）

### [x] F7.1 Sidebar 批量分析首条进度说明（P2）（commit `eb07267`）

- **根因/现状（已核实）**：`analyzeBatchCore` 在同一同步段连续报告 `Analyzing N chunks…` 与 `Analyzing chunk 1/N…`；VS Code Notification 只渲染最后一条，因此用户看不到总邮件数、chunk 数和开始提示。
- **做法**：首个 chunk 的单条 progress 改为同时说明实际 batch 邮件数、拆分的 chunk 数及 `Starting chunk 1/N`；后续 chunk 继续保留当前剩余时间预估。不得人为 sleep 或推迟模型请求，不改 chunk 切分/实际 batch 选择语义。
- **验收**：单测锁定首条完整文案与后续预估；`npm test` 全绿。**needs user validation**：从 Sidebar 选择 5/10/20 或 All 分析，首条能说明实际邮件数与 chunks，随后进度正常推进。

- **Completion Notes**：
  - 改动文件：`src/lib/app-analysis.ts`、`src/test/app-analysis.test.ts`。
  - 实现边界：删除会被下一条同步覆盖的独立总量提示，将实际参与分析的邮件数、chunk 数和 `Starting chunk 1/N` 合并为首条可见进度；后续预估不变。未增加延时、未改变模型请求、chunk 切分、selection 或 Sidebar 刷新。
  - 验收结果：首条进度断言先 RED 后 GREEN；定向 app-analysis 27/27 通过；`npm run compile` 零错误；完整 `npm test` 429/429 通过；`git diff --check` 通过。
  - Manual validation：**needs user validation on real VS Code**。从 Sidebar 分析 5/10/20 或 All，确认首条为类似 `Analyzing 20 emails in 2 chunks. Starting chunk 1/2…`，随后继续显示后续 chunk 的剩余时间预估。
  - Known issues：无新增。
  - Commit：`eb07267`。
...
### [x] F7.4 VS Code Settings 收件人配置相邻排序（P2）（commit `edcc2f3`）

- **现状（已核实）**：`easyMail.folders`、`easyMail.importantSenders` 与 `easyMail.ignoredSenders` 的 manifest order 分别为 4、10、17，VS Code Settings 中不相邻。
- **做法**：仅调整这三项 order，使顺序为 Folders → Important Senders → Ignore Senders；不重排或修改任何其他 setting。
- **验收**：manifest 回归断言锁定三项顺序；`npm test` 全绿。**needs user validation**：在 VS Code Settings 的 EasyMail 分组中确认三项相邻且顺序正确。

- **Completion Notes**：
  - 改动文件：`package.json`、`src/test/sidebar-render.test.ts`。
  - 实现边界：仅将 Important Senders、Ignore Senders 的 configuration order 设为 4.1、4.2，使其紧邻 Folders 的 4；字段、默认值、description 和其他 setting order 均未改动。
  - 验收结果：manifest order 断言先 RED 后 GREEN；定向 sidebar render 43/43 通过；`npm run compile` 零错误；完整 `npm test` 432/432 通过；`git diff --check` 通过。
  - Manual validation：**needs user validation on real VS Code**。打开 Settings 的 EasyMail 分组，确认 Folders、Important Senders、Ignore Senders 连续显示且顺序正确。
  - Known issues：无新增。
  - Commit：`edcc2f3`。
